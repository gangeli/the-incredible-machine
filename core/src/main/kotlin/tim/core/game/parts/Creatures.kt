...
abstract class Creature(placement: Placement, index: Int, val mass: Double, val speed: Double, val accel: Double, private val strideLength: Double) : Part(placement, index) {
    ...
    fun noise(seed: Int): Double {
        val v = StrictMath.sin(seed * 12.9898 + index * 78.233 + 1.0) * 43758.5453
        return v - StrictMath.floor(v)
    }
    ...
    override fun postStep() {
        if (caught) return
        val dt = World.STEP
        stride += abs(body.vel.x) * dt / strideLength * 2 * Math.PI
...
class Mouse(placement: Placement, index: Int) : Creature(placement, index, 0.1, 70.0, 900.0, 9.0) {
    ...
    override fun draw(p: Painter, t: Double) {
        ...
        val bob = if (run) abs(StrictMath.sin(stride)) * 1.2 else 0.0
        val nod = if (sniffing) StrictMath.sin(tt * 14) * 0.8 else if (eating) StrictMath.sin(tt * 16) * 1.4 else 0.0
        val stretch = if (scared) 1.12 else 1.0
        val bodyY = -6.5 - bob
        // tail: sways while running, hangs still while sniffing, up in the air
        val sway = if (run) StrictMath.sin(stride * 0.5) * 3 else StrictMath.sin(tt * 1.5) * 1.5
        val tail = Path()
        ...
        for ((i, lx) in listOf(-7.0, -3.0, 2.0, 6.0).withIndex()) {
            val phase = stride * 1.6 + i * Math.PI / 2
            val swing = if (run) StrictMath.sin(phase) * 2.5 else if (airborne) (if (i < 2) -2.5 else 2.5) else 0.0
            val lift = if (run) max(0.0, -StrictMath.cos(phase)) * 1.5 else 0.0
        ...
        val twitch = if (sniffing) StrictMath.sin(tt * 20) * 1.0 else 0.0
    ...
    override fun draw(p: Painter, t: Double) {
        ...
        if (sit) {
            ...
            for ((i, hip) in listOf(-12.0, -7.0, 7.0, 12.0).withIndex()) {
                val phase = stride + (if (i == 0 || i == 3) 0.0 else Math.PI)
                val swing = if (run) StrictMath.sin(phase) * 6 else 0.0
                val lift = if (run) max(0.0, -StrictMath.cos(phase)) * 4 else 0.0
