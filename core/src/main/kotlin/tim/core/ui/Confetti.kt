...
class Confetti(private val w: Double, private val h: Double, seed: Long = 7) {
    ...
    fun burst(cx: Double, cy: Double, n: Int = 90) {
        repeat(n) {
            val a = rng.range(-Math.PI, 0.0)
            val sp = rng.range(h * 0.5, h * 1.4)
            parts.add(P(cx, cy, StrictMath.cos(a) * sp, StrictMath.sin(a) * sp, rng.range(h * 0.008, h * 0.018), colors[(rng.next() * colors.size).toInt()], rng.range(0.0, 6.28), rng.range(-6.0, 6.0)))
        }
    }
